...
fn foo() -> Result<(), RclrsError> {
    ...
    let _node_1_basic_types_publisher = node1
        .create_publisher::<msg::BasicTypes>("graph_test_topic_1", QOS_PROFILE_SYSTEM_DEFAULT)?;
    ...
    let _node_1_empty_service =
        node1.create_service::<srv::Empty, _>("graph_test_topic_3", |_request_id, _srv| {
            srv::Empty_Response {
    ...
    let _node_2_default_publisher = node2
        .create_publisher::<msg::Defaults>("graph_test_topic_2", QOS_PROFILE_SYSTEM_DEFAULT)?;
    ...
    let _node_2_empty_client = node2.create_client::<srv::Empty>("graph_test_topic_3")?;
...
fn check_publishers(node1: &Node, node2: &Node) -> Result<(), RclrsError> {
    ...
    assert_eq!(node1.count_publishers("/graph_test_topic_1")?, 2);
    ...
    let types = node_2_publisher_names_and_types
        ...
        .unwrap();
    assert_eq!(node2.count_publishers("/graph_test_topic_2")?, 1);
...
fn check_subscriptions(node1: &Node, node2: &Node) -> Result<(), RclrsError> {
    ...
    assert_eq!(node1.count_subscriptions("/graph_test_topic_2")?, 1);
    assert!(types.contains(&"test_msgs/msg/Defaults".to_string()));
    ...
    assert_eq!(node2.count_subscriptions("/graph_test_topic_1")?, 2);
    assert!(types.contains(&"test_msgs/msg/BasicTypes".to_string()));
    assert!(types.contains(&"test_msgs/msg/Empty".to_string()));
...
fn check_services(node: &Node) -> Result<(), RclrsError> {
    let check_names_and_types = |names_and_types: TopicNamesAndTypes| {
        let types = names_and_types.get("/graph_test_topic_3").unwrap();
...
fn check_clients(node: &Node) -> Result<(), RclrsError> {
    ...
    let types = client_names_and_types.get("/graph_test_topic_3").unwrap();
